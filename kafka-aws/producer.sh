...
oc run kafka-producer -n myproject -ti --image=strimzi/kafka:0.12.1-kafka-2.2.1 --rm=true --restart=Never -- bin/kafka-console-producer.sh --broker-list ${nodeip}:${nodeportbs},${nodeip}:${nodeport0},${nodeip}:${nodeport1},${nodeip}:${nodeport2} --topic my-topic1
