...
sed -e "s/<clusterID>/${clusterid}/g" -e "s/<role>/${role}/g" -e "s/<INSTANCETYPE>/${instancetype}/g" -e "s/<REGION>/${region}/g" -e "s/<ZONE>/${zone}/g" -e "s/<AMIID>/${ami_id}/g" -e "s/<REPLICAS>/${desired_replicas}/g" machineset.template.yaml > generated/$clusterid-$role-$region$zone.yaml
